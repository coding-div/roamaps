...
export default function NodePopup({ node, treeId, dispatch, onClose, switchTarget, onSwitchTargetHandled, onSwitchTo }: NodePopupProps) {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/65 px-4 py-6" role="dialog" aria-modal="true" aria-label={`${heading} popup`}>
      <div className="relative flex max-h-[calc(100vh-3rem)] w-full max-w-2xl flex-col overflow-hidden rounded-2xl border border-[#30303d] bg-[#111118] shadow-[0_24px_90px_rgba(0,0,0,0.6)]" onPointerDown={(event) => event.stopPropagation()}>
        <div className="flex shrink-0 items-center justify-between border-b border-[#2a2a35] px-5 py-4">
          <div className="min-w-0 pr-4"><p className="font-mono text-[10px] uppercase tracking-[0.24em] text-[#77809a]">Node document</p><h2 className="mt-1 truncate font-sans text-lg font-semibold text-[#f2f4fa]">{heading}</h2></div>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5 py-5">
          {editing ? <textarea ref={textareaRef} autoFocus value={draft} onChange={(event) => setDraft(event.target.value)} placeholder="Write your notes here..." className="min-h-[320px] w-full resize-none overflow-y-auto rounded-xl border border-[#30303d] bg-[#0a0a0f] px-4 py-4 font-sans text-[15px] leading-7 text-[#e4e4e7] outline-none placeholder:text-[#555564] focus:border-[#4c7dff]" aria-label="Node document text" /> : savedContent ? <p className="whitespace-pre-wrap break-words font-sans text-[15px] leading-7 text-[#d5d7df]">{savedContent}</p> : <div className="flex min-h-[300px] flex-col items-center justify-center text-center"><FileText className="h-8 w-8 text-[#4c7dff]/65" /><p className="mt-3 font-sans text-sm text-[#7f8494]">No notes yet</p><p className="mt-1 max-w-xs font-mono text-[10px] uppercase tracking-[0.16em] text-[#4b4d58]">This document belongs to {heading}</p></div>}
        </div>

        <div className="flex shrink-0 items-center justify-between gap-3 border-t border-[#2a2a35] px-5 py-4">
          <p className="font-mono text-[10px] uppercase tracking-[0.16em] text-[#545866]">{editing ? "Local editor history active" : "Plain text document"}</p>
          {editing ? <div className="flex gap-2"><button onClick={closeRequested} className="rounded-lg border border-[#2a2a35] px-3 py-2 text-xs text-[#a7a7b3] hover:border-[#4b4d58] hover:text-white active:scale-95">Cancel</button><button onClick={saveChanges} className="flex items-center gap-1.5 rounded-lg bg-[#4c7dff] px-3 py-2 text-xs font-medium text-white hover:bg-[#3c6bea] active:scale-95"><Save className="h-3.5 w-3.5" />Save</button></div> : <button onClick={() => { setDraft(savedContent); setEditing(true); }} className="flex items-center gap-1.5 rounded-lg bg-[#4c7dff] px-3 py-2 text-xs font-medium text-white hover:bg-[#3c6bea] active:scale-95"><Check className="h-3.5 w-3.5" />Edit Text</button>}
        </div>
