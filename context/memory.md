...
## Arrow-stage issue revision opened — 2026-08-14

The user now wants a focused arrow-stage revision. The six reported requirements are: (1) restore arrowheads that disappear into or behind a normal node; (2) after removing a joiner, support a deliberate multi-headed visual result rather than accidental overlapping arrows sharing one path; (3) when a target node in that result moves, change only its affected branch while keeping the shared tail anchored at the joint; (4) never allow two arrows to overlap—routes may approach, but a forced collision should restore the dragged node to the other valid side; (5) treat tails from multiple arrows entering the same node and every parallel segment of an arrow as routing obstacles to one another; and (6) preserve bridge arches for perpendicular crossings.

Initial code diagnosis, not yet a fix: `getArrowHead()` currently places one polygon at the final route point, while edges render before nodes, so a head at or just inside a node boundary can be covered by the later node rectangle. `REMOVE_NODE` currently reconnects every incoming source to every outgoing target as independent `ChildRef` edges; this explains why removing a joiner can produce separate arrowheads that visually share a corridor. The route builder currently avoids normal-node boxes but does not treat existing arrow segments as obstacles, and bridge marks are drawn after route calculation rather than being used to prevent parallel overlap.

Open semantic decision before the arrow contract: clarify whether the requested “multi-headed arrow” means one bundled visual route with one shared tail and multiple independently anchored heads, or several distinct arrow objects that must remain visually separated by lanes while still sharing a logical joint. No arrow code has been changed for this stage.
